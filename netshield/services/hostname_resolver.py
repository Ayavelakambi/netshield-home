...
def public_resolve_a(domain: str, resolver_ip: str = "1.1.1.1",
                     timeout: float = 2.5) -> list[str] | None:
    """Resolve a domain's IPv4 addresses directly against a public resolver.

    Used by the DNS-bypass feature: when the router/firewall blocks a domain
    in its own DNS, NetShield asks the public resolver itself and answers
    the device — the router's DNS filter is never consulted.
    """
    try:
        ...
        ips: list[str] = []
        for _name, rtype, _ttl, rdata in parsed.get("answers", []):

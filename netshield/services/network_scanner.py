    ...
    for info in found:
        ...
        if is_new:
            try:
                ...
                # auto port-scan new devices when enabled (Settings)
                try:
                    from netshield.models.models import Setting as _St
                    if bool(_St.get("auto_scan_new", False)):
                        from netshield.services import port_scanner
                        port_scanner.scan_device(device)
                except Exception:
                    pass
            except Exception as exc:
                logger.debug("new-device alert failed: %s", exc)
        # apply desired control/capture to this device immediately, so
        # network-wide blocks and capture modes cover NEW devices right
        # away (the scheduler also re-applies every minute)
        if device.control_state != "none" or device.current_ip:
            try:

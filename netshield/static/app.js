...
(function () {
  ...
  socket.on("hostname", function (h) {
      var cell = document.querySelector('td[data-mac="' + h.mac + '"]');      if (cell) {
  ...
  socket.on("network_reset", function (n) {
    // the host switched to a different Wi-Fi/router — the app wiped the old
    // network's data; reload so the UI shows the fresh (empty) network
    toast("Network changed to " + (n.network || "a new network") +
      " — previous devices and history cleared. Rescanning now.", "warn");
  ...
  if (themeBtn) {
    ...
    try { applyTheme(localStorage.getItem("ns-theme") || "dark"); } catch (e) {}
    themeBtn.addEventListener("click", function (e) {
      ...
      applyTheme(next);
      try { localStorage.setItem("ns-theme", next); } catch (e2) {}
    });
  }
})();
